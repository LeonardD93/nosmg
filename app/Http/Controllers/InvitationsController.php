...
class InvitationsController extends Controller
{
    public function store(Request $request)
    {
        ...
        $invitation->invited_by_user=$user_id;      
        $invitation->save();
    ...
    public function index(){
        $user = Auth::user();       
        if(!$user->isAdmin())
            $invitations = Invitation::where('registered_at', null)->where('invited_by_user', $user->id)->orderBy('created_at', 'desc')->get();// vedo solo gli inviti che ho mandato      
        else  
    ...
    public function destroy($id){
        ...
        if($user->isAdmin())
            ...
            $invitation=Invitation::where('id',$id)->where('invited_by_user',$user->id);
